...
function LinkedInPage() {
  ...
  const insertDataToDatabase = async (isAgree) => {
    try {
      setIsDataInserting(true);
      const datacube = new Datacubeservices(
        "1b834e07-c68b-4bf6-96dd-ab7cdc62f07f"
      );
      const databaseName = "EXPERIENCE_PRODUCT_DB0";
      const collectionName = "DOWELL_ANALYTICS_PRODUCT_DATA";

      const dataToInsert = {
        client_email: email,
        client_linkedin_link: linkedInLink,
        price: price,
      ...
      await datacube.dataInsertion(databaseName, collectionName, dataToInsert);

      return true;
    } catch (error) {
      console.error("Error inserting data to database:", error);
      toast.error("Failed to record user action. Please try again.");
      return false;
    } finally {
  ...
  const handleEmail = async (data) => {
    ...
    const feedbackBaseUrl =
      "https://www.scales.uxlivinglab.online/api/v1/create-response/?user=True&scale_type=nps&channel=channel_1&instance=instance_1&workspace_id=6385c0e48eca0fb652c9447b&username=HeenaK&scale_id=665d95ae7ee426d671222a7b&item=";
  ...
  <html>
    ...
    <body>
      <div class="header">
        ...
        <p class="date">Generated on ${new Date().toLocaleDateString("en-GB", {
          day: "numeric",
          month: "long",
          year: "numeric",
        })}</p>
      ...
      ${analysisData
        ...
        <div class="swot-section ${
          section.heading.includes("Overall Grade") ? "overall-grade" : ""
        }">
...
`;
    ...
    try {
      const emailData = {
        ...
        toemail: email,
        subject:
          "🚀 LinkedIn Analysis from Samanta AI... Unlocking Valuable Insights",
  ...
  async function handleCancel() {
    const isDataInserted = await insertDataToDatabase(false);
    if (!isDataInserted) {
      toast.error(
        "Failed to insert data into database. Please try again later."
      );
    }
  ...
  //       } catch (error) {
  //         console.error("Error in analyzing LinkedIn profile.", error);
  ...
  //     }
  //   } catch (error) {
  //     toast.error("Error in analyzing LinkedIn profile.",error)
  ...
  async function handleAgree() {
    // First, insert data with isAgree true
    const isDataInserted = await insertDataToDatabase(true);
          ...
          {isEligible !== null && !showPaymentPopup && (
            <div>
              ...
              {!isEligible && (
                <button
                  onClick={handlePrice}
                  className="bg-red-400 w-full p-2 rounded-2xl text-white font-semibold"
                >
                  Contribute
                </button>
              )}
                ...
                <div className="p-6">
                  <div className="flex flex-col items-center mb-4">
                    <p className="text-gray-700">
                      To access the in-depth insights from Samanta LinkedIn
                      Analysis, a fee is required. You'll receive a detailed
                      invoice and a secure payment link via email, along with
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 mt-6 justify-center">
                    ...
                    <button
                      ...
                      onClick={handleAgree}
                      disabled={!price || isDataInserting}
                    >
                      {isDataInserting ? (
                        <Loader2 className="animate-spin mr-2" />
                      ) : (
