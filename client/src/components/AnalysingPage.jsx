    ...
    async function analyze() {
      try {
        ...
        const swotResponse = await sowtAnalysis({
          ...
          occurrences: occurrences,
>>>>>>> 34485a3cdba86c237dc6f36806e6501438288f0d
        });
        setSwotAnalysisData(swotResponse.response);
        setIsAnalysing(false);
        setIsAnalysisComplete(true);
...
>>>>>>> 34485a3cdba86c237dc6f36806e6501438288f0d
    ...
    try {
      const emailData = {
        ...
        toemail: email,
        subject:
          "🚀 Unlock Data-Driven Business Insights with Samanta AI – Get Customer perspective",
      ...
      <div className="flex flex-col space-y-3 px-15">
        {isAnalysing ? (
          <p className="text-red-500 text-sm">
            ...
            {isAnalysing && <Loader2 size={12} className="animate-spin flex" />}
          </p>
        ) : (
          <p className="text-sm">Report prepared.</p>
        )}
        <div className="border rounded-2xl w-full h-full overflow-hidden min-h-60 min-w-50">
          {analysisData && (
            <MapComponent
              lat={analysisData.latitude}
              lng={analysisData.longitude}
            />
          )}
>>>>>>> 34485a3cdba86c237dc6f36806e6501438288f0d
        </div>
      ...
      {isAnalysisComplete && (
        <div className="flex flex-col space-y-2">
          ...
          />
          <button
            className="bg-green-500 p-2 rounded-2xl w-full flex items-center justify-center gap-2"
            onClick={handleEmail}
          >
            <span>Send</span>
      ...
      <div className="relative flex pr-5 pb-5 overflow-hidden">
        <div className="flex-grow">
          <p className="absolute left-0 z-50 w-40">
            Samanta will tell the story on analysed data
          </p>
          <img
            src={DowellLogo}
            alt="Noo"
            className="absolute z-50 w-10 h-10 left-0 bottom-5"
          />
        </div>
        <img src={SamantaLogo} alt="" className="w-30 h-30 scale-180" />
      </div>
